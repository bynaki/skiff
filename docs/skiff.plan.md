...
## 3. 쓰이지 않는 표면: 연결하든 지우든 결정할 것 — 끝남 (2026-09-25)

존재하는데 아무도 안 쓰는 게 둘 있다. **각각 배선하거나 삭제해야 한다 — 지금처럼 두는 게 둘
중 어느 쪽보다도 나쁘다.**

- **`FileSystem.freeSpace`** — 배선했다(2026-09-25). `CopyEngine.plan`이 트리를 다 센 뒤, 옮길
  바이트가 목적지의 여유 공간보다 크면 바이트가 움직이기 전에 `FsError.NoSpace`를 던진다.
  `SftpFileSystem.freeSpace`는 의도적으로 null이라(SFTP 기본 프로토콜에 해당 호출이 없다) **로컬이
  목적지일 때만** 점검한다. 덮어쓰기로 풀려날 공간은 셈하지 않으므로, 꽉 찬 저장소에서 같은 크기의
  파일을 덮어쓰는 전송은 실제로는 들어갈 수 있어도 거절된다. 기기에서는 확인하지 않았다.
