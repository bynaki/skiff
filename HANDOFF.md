...
## 마지막 세션 (2026-09-24, 네 번째): 사용자 요청 세 건 — 빈 파일, `.jsonl`, 로딩 표시

### 지금 상태
- 브랜치 `plan/skiffcode`(git worktree). **이번 세션 커밋은 넷**이고 **push했다** — origin도 `7fa958d`,
  워킹 트리는 이 문서를 고친 커밋 말고 깨끗하다.
  - `f3d1613` Skiff(`:app`) 툴바의 "새 파일" 버튼
  - `642a8a5` `.jsonl`을 JSON 문법으로
  - `a63fb0b` 링크를 여는 동안의 로딩 표시
  - `7fa958d` 상단바 파일 이름과 점 — **설계와 할 일만, 구현 없음**
- **`plan.md`의 첫 `- [ ]`는 여전히 커맨드 레지스트리 ②/③이다.** 이번 세션은 거기 손대지 않았다 — 사용자
  요청 셋이 먼저 들어왔고, 셋 다 M4 목록 안에 항목으로 들어가 있다.
- **M4 끝에 `- [ ]`가 하나 늘었다**: 상단바 ① 옆 파일 이름과 저장 상태 점. 설계는 `plan.md`의
  "화면 메뉴"에 있고, 건드릴 파일 다섯 곳과 확인 기준까지 그 항목에 적혀 있다.
- **main 병합 조건은 지난 세션 그대로다**(②③과 그다음 확인 항목이 끝난 뒤에 묻는다). 브랜치가 앞서 있고
  main에만 있는 것은 없어 fast-forward인 것도 그대로. 이 worktree에서 `git checkout main`은 하지 않는다.
- 테스트: 페이지(vitest) **60개**(지난 세션 53 + 언어 2 + 로딩 5). `:app`·`:code`의 유닛 테스트와
  `:app:assembleDebug`·`:code:assembleDebug`도 통과. **lint는 이번에 돌리지 않았다.**
- 기기: **탭(SM-X526N)만 USB로 붙어 있다.** Skiff와 Skiff Code 둘 다 이번 빌드가 깔려 있다.
  **폴드8(SM-F971N)은 세션 중간에 빠졌다** — Skiff는 `f3d1613`까지 깔았지만 **Skiff Code의 로딩 표시는
  깔려 있지 않다.** APK(`code/build/outputs/apk/debug/code-debug.apk`)를 대화로 보내 두었다.
- 기기에 남긴 것 없음. 확인용 파일도 만들지 않았다(전부 원래 있던 파일과 화면 캡처로 봤다).

### 이번 세션에서 한 것
설계와 이유는 `plan.md`에 적었다. 여기에는 그 문서에 없는 것만 남긴다.

**1. Skiff(`:app`) — "새 파일"을 툴바로**
- **기능은 이미 전부 있었다.** `FileSystem.createFile`(로컬은 `createNewFile`, 원격은 `CREAT|EXCL`),
  `WorkspaceViewModel.createFile`, 이름 대화상자까지 첫 커밋들부터 있었고 **오버플로 메뉴(⋮) 안에 숨어
  있었을 뿐이다.** 사용자가 "기능을 넣어달라"고 한 것도 그래서였다 — 찾지 못했던 것이다.
- 그래서 바꾼 것은 `WorkspaceScreen.kt` 한 곳뿐이다. 툴바에 아이콘 버튼을 더하고 메뉴의 같은 항목은 뺐다.
  같은 동작이 두 자리에 있을 이유가 없다.

**2. `.jsonl`**
- 사용자가 **뷰어만** 고쳤다(2026-09-23 결정). `FileKind`를 건드리지 않았으므로 **Skiff 목록에서 `.jsonl`을
  탭하면 여전히 밖으로 나가고** 아이콘도 일반 파일이다. 강조는 링크로 연 Skiff Code 안에서만이다.
- `@codemirror/lang-json`을 직접 의존하지 않았다(지금은 language-data의 전이 의존이다). 이름의 끝 `l`을
  떼어 language-data의 JSON 항목이 답하게 하는 두 줄이 전부다.

**3. 로딩 표시**
- **`openingChanged` 알림이 이번에 새로 생긴 브리지 신호다.** 페이지가 기다림을 볼 수 있는 유일한 창이다.
- 규칙(`loading.ts`)과 그리기(`chrome/loading.ts`)를 나눈 것은 팔레트와 같은 이유다 — vitest가 DOM 없이
  규칙만 본다. 테스트 5개가 그 규칙이다.
- **아직 기기에서 눈으로 못 봤다.** 아래 "아직 확인하지 않은 것"에 적었다.

**4. 상단바 파일 이름과 점 — 설계만**
- 사용자가 **구현 전에 보고부터** 요구했고, 보고 뒤 "설계 파일에 적고 할 일에 넣어두라"고 했다.
- 설계에서 나온 사실 하나: **"수정됨"이 바뀌었다는 신호가 지금 없다.** `dirtyFlag`는 `EditorState`에 있어
  물어볼 수만 있고, 사이드바도 닫을 때 물어볼 뿐 표시하지 않는다. 그래서 `openPane`에 콜백을 더해
  `updateListener`에서 뒤집힐 때만 부르는 것이 이 항목의 실제 작업이다.
- 점은 **수정됐을 때만** 둔다(2026-09-24 사용자 결정). 늘 두고 색으로 나누는 안은 버렸다.

### 실기기에서 본 것
- **탭:** Skiff 상단바에 새 파일 아이콘이 새 폴더 옆에 붙었다(화면 캡처로 확인). 아이콘 7개가 가로 화면에
  여유 있게 들어간다.
- **폴드8(커버 화면):** 같은 툴바가 **잘림 없이 7개 다** 들어갔다. 다만 제목 "Skiff" 오른쪽 여백이 거의
  없어서 **전송 배지(숫자)가 붙으면 빠듯해질 수 있다** — 사용자가 확인은 생략하라고 했다.
- **로딩 표시는 보지 못했다.** 로컬 파일은 200ms 안에 끝나 설계대로 아무것도 뜨지 않는다.
...
3. 그다음은 사용자에게 묻는다 — **상단바 파일 이름과 점**(설계가 준비돼 있다)이냐,
   **커맨드 레지스트리 ②/③**(`plan.md`의 첫 `- [ ]`)이냐.
4. **폴드8에 Skiff Code를 깐다.** 기기가 Wi-Fi에 붙으면 무선 디버깅으로 된다 — **페어링은 이미 돼 있다**
   (기기의 "페어링된 기기"에 이 맥이 있다). 자세한 것은 아래 "주의할 점".
...
| `.jsonl` 지원 범위 (2026-09-23) | Skiff Code 뷰어의 문법만. `FileKind`는 그대로 둔다 | 사용자 결정. Skiff 목록에서 탭하면 여전히 밖으로 나간다 |
| 로딩 표시 (2026-09-24) | 위 라인 + 문서가 없을 때만 스켈레톤. 200ms 전에는 아무것도, 뜨면 300ms 유지 | 사용자 결정. 네이티브 오버레이는 기존 다이얼로그와 부딪히고, 퍼센트는 정작 긴 구간(접속)에 줄 것이 없다 |
| 저장 상태 점 (2026-09-24) | 수정됐을 때만 점. 저장된 상태에는 아무것도 없다 | 사용자 결정. 늘 두고 색으로 나누면 아무 일 없는 동안에도 화면에 하나가 더 있다 |
