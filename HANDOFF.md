...
### 지금 상태
- 브랜치 `plan/skiffcode`(git worktree). **이번 세션의 작업은 아직 커밋하지 않았다**(사용자에게 묻고 한다).
  직전 커밋 `14a5447`까지는 origin에 올라가 있다 — 로컬 기준으로 push할 것이 없는 상태에서 시작했다.
- **`plan.md`의 첫 `- [ ]`는 이제 M3의 마지막 항목인 확인**이다. LRU 항목은 체크했다.
...
2. **`plan.md`의 첫 `- [ ]`인 M3의 확인 항목**을 한다 — "서버에서 `echo >> file` 하면 2초 안에
   반영되고, 수정 중에는 배너가 뜬다. 저장 후 서버의 `git diff`에 의도한 변경만 보인다".
   - **앞의 절반은 지난 세션에 이미 봤다**(원격 2.05초, 배너, 내 것 유지/다시 불러오기).
   - **뒤의 절반은 지금 할 수 없다.** 저장을 거는 UI가 M4의 커맨드 레지스트리라 `DocumentSaver`를
     앱에서 부를 길이 없다. 사용자에게 **이 항목을 M4로 미룰지** 묻고 시작하는 것이 좋겠다.
3. 그다음은 M4의 첫 항목(커맨드 버튼 A/B/C 상태 기계 + vitest)이다. **vitest는 이제 있다.**
