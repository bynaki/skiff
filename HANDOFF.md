...
- 브랜치 `plan/skiffcode`(git worktree). **이번 세션 커밋은 둘이다** — `0c6bd16`(팔레트 A/B/C 상태 기계)과
  `b7e29b6`(스와이프·퍼지 점수·최근 목록·가로 상한·아이콘). **둘 다 push했다.** origin도 `b7e29b6`이고,
  워킹 트리는 이 문장을 고친 커밋 말고는 깨끗하다.
- **M4의 첫 두 항목이 끝났고, `plan.md`의 첫 `- [ ]`는 M4의 커맨드 레지스트리**(레이어 전환·확대/축소·저장·
  사이드바·설정 열기 + 파일 모드 + lezer 심볼 모드)다.
- **`main`에는 아직 합치지 않았다. 커맨드 레지스트리 항목이 끝난 뒤에 합친다**(2026-09-21 사용자 결정).
  브랜치가 main보다 68 커밋 앞서고 **main에만 있는 것은 없어서 fast-forward**다. 지금 합치지 않는 이유는
  저장이다 — `DocumentSaver`는 다 만들어져 있지만 그것을 부르는 커맨드가 그 항목이라, 지금 합치면 main의
  Skiff Code는 저장할 수 없는 에디터다. 태그도 CI도 없다.
  - **이 worktree에서 `git checkout main`을 하지 않는다**(메인 체크아웃이 그 브랜치를 쥐고 있다).
    원격을 먼저 올리는 `git push origin plan/skiffcode:main`이 맞고, 로컬 main은 메인 체크아웃에서 `pull`한다.
...
2. **`plan.md`의 첫 `- [ ]`인 커맨드 레지스트리**를 한다. 레이어 전환·확대/축소·**저장**·사이드바·설정 열기와
   ...
   - **그 항목이 끝나면 `main` 병합을 물어본다.** 위 "지금 상태"에 방법이 있다.
3. 사용자가 물어 둔 것 하나가 열려 있다: **확대 배율을 `localStorage`에 넣을지** — 규칙상 넣는 게 맞고
...
| **`main` 병합 (2026-09-21)** | **커맨드 레지스트리 항목이 끝난 뒤에 합친다** | 사용자 결정. 그 항목이 저장 커맨드를 붙이기 전까지는 main의 Skiff Code가 저장할 수 없는 에디터다. fast-forward라 언제 해도 충돌은 없다 |
